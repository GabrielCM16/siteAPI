...
//pega a variavel com as moedas e coloca em um array
function moedasParaList(str) {
...
const moedasArray = moedasParaList(moedas);
console.log(moedasArray);
console.log(moedasArray.length);


//precisa setar as opcoes de moedas
function colocaOpcoes(opcoesArray) {
  var select1 = document.getElementById("opcoes1");
  var select2 = document.getElementById("opcoes2");
  // Adiciona as opções na caixa de seleção
  for (var i = 0; i < opcoesArray.length; i++) {
      var option = document.createElement("option");
      option.text = opcoesArray[i];
...
colocaOpcoes(moedasArray);


//botao colar
async function cotar(){
  limpaCampos();
  var opcao1 = document.getElementById("opcoes1");
  var opcao2 = document.getElementById("opcoes2");

  var moeda1 = opcao1.options[opcao1.selectedIndex].text;
  var moeda2 = opcao2.options[opcao2.selectedIndex].text;

  var moedaPesquisa = "";

  var aux1 = moeda1.split(" ");
  var aux2 = moeda2.split(" ");

  moedaPesquisa = aux1[0] + "-" + aux2[0];
  console.log(moedaPesquisa);

    const url = `https://economia.awesomeapi.com.br/last/${moedaPesquisa}`;
        ...
        if (cotacao.status == "404") {
          alert("Moeda não encontrada,\n Selecione Outra");
            
        } else {
          var aux3 = moedaPesquisa.replace('-', '');
            mostrarMoeda(cotacao[aux3], aux1[0], aux2[0]);
        }

}

document.getElementById('idbotao').addEventListener('click',cotar);


function limpaCampos() {
  document.getElementById('saidaNome').innerHTML = '';
  document.getElementById('saidaAlta').innerHTML = '';
  document.getElementById('saidaBaixa').innerHTML = '';
  document.getElementById('saidaPorcemtagemVariacao').innerHTML = '';
  document.getElementById('saidaCompra').innerHTML = '';
  document.getElementById('saidaVenda').innerHTML = '';
  document.getElementById('saidaVariacao').innerHTML = '';
}

function mostrarMoeda(moeda, moeda11, moeda22) {
  document.getElementById('saidaNome').innerHTML = moeda.name;
  document.getElementById('saidaAlta').innerHTML = "1 " + moeda11 + " = " + moeda.high + " " + moeda22;
  document.getElementById('saidaBaixa').innerHTML = "1 " + moeda11 + " = " + moeda.low + " " + moeda22;
  document.getElementById('saidaPorcemtagemVariacao').innerHTML = moeda.pctChange + "%";
  document.getElementById('saidaCompra').innerHTML = moeda.bid;
  document.getElementById('saidaVenda').innerHTML = moeda.ask;
  document.getElementById('saidaVariacao').innerHTML = moeda.varBid + "%";
  document.getElementById('saidaData').innerHTML = moeda.create_date;
